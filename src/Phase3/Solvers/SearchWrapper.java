...
public class SearchWrapper {
    ...
    public static void addPiece(int[][][] gameField, boolean[][][] piece, int[] coordinates, int pieceID) {
        for (int z = 0; z < piece.length; z++) {
            for (int y = 0; y < piece[z].length; y++) {
                for (int x = 0; x < piece[z][y].length; x++) {
                    if (piece[z][y][x]) {
                        gameField[coordinates[0] + z][coordinates[1] + y][coordinates[2] + x] = pieceID;
                    }
                }
            }
        }
    ...
    public static boolean canPlacePiece(int[][][] field, boolean[][][] piece, int[] placementCoords) {
        for (int z = 0; z < piece.length; z++) {
            for (int y = 0; y < piece[z].length; y++) {
                for (int x = 0; x < piece[z][y].length; x++) {
                    if (!piece[z][y][x]) {
